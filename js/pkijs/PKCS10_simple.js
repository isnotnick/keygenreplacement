    ...
    */
	...
	function createPKCS10Internal() {
		...
		pkcs10.subject.typesAndValues.push(new AttributeTypeAndValue({
			type: "2.5.4.6",
			value: new PrintableString({ value: "GB" })
		}));
		pkcs10.subject.typesAndValues.push(new AttributeTypeAndValue({
			type: "2.5.4.3",
			value: new Utf8String({ value: "Keygen-replacement generated" })
		}));
		...
		sequence = sequence.then(function () {
			...
			var pKeyString = "";
			pKeyString = pKeyString + "-----BEGIN PRIVATE KEY-----\r\n";
			pKeyString = pKeyString + formatPEM(window.btoa(private_key_string));
			pKeyString = pKeyString + "\r\n-----END PRIVATE KEY-----";
			//document.getElementById("pkey-text-block").value = pKeyString;

			localforage.setItem('codesigningKey', pKeyString).then(function (value) {
				//console.log(value);
			}).catch(function(err) {
				console.log(err);
			});
		}, function (error) {
			return Promise.reject("Error during exporting private key: " + error);
		});
		// #endregion
	...
	function createPKCS10() {
		return Promise.resolve().then(function () {
			...
			document.getElementById("1_csr").value = resultString;
			localforage.setItem('codesigningCSR', resultString).then(function (value) {
				//console.log(value);
			}).catch(function(err) {
				console.log(err);
			});
			//parsePKCS10();
			document.getElementById('codesigningForm').submit();
		});
	}
	//*********************************************************************************
	//endregion
